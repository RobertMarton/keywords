...
from xlnet.classifier_utils import convert_keyword_single_example
...
flags.DEFINE_string("init_checkpoint",
                    default="gs://ccf_cp/ccf/pretrain_model/chinese_xlnet_mid_L-24_H-768_A-12/xlnet_model.ckpt",
                    help="checkpoint path for initializing the model. "
                    "Could be a pretrained model or a finetuned model.")

flags.DEFINE_string("output_dir",
                    default="gs://ccf_cp/output_xlnet",
                    help="Output dir for TF records.")

flags.DEFINE_string("spiece_model_file",
                    default="xlnet/spiece.model",
                    help="Sentence Piece model path.")

flags.DEFINE_string("model_dir",
                    default="gs://ccf_cp/model_dir_xlnet",
...
class ccfKeywordProcessor(DataProcessor):
    def get_train_examples(self, data_dir):
...
def file_based_convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        feature = convert_keyword_single_example(ex_index, example, label_list, max_seq_length, tokenize_fn)
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        for global_step, filename in sorted(
            ...
            result = estimator.predict(input_fn=pred_input_fn, checkpoint_path=filename)

            output_predict_file = os.path.join(FLAGS.predict_dir, str(global_step) + "_test_results.tsv")
