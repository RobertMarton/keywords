...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    with tf.variable_scope("loss"):
        ...
        if FLAGS.use_multi_loss:
            ...
            # focal loss
            fl_loss = -tf.reduce_sum(one_hot_labels * (1 - probabilities) ** 0.5 * log_probs)
            tf.logging.info("**** fl loss ****: " + str(fl_loss))

            # hige loss
            hg_loss = tf.reduce_sum(tf.losses.hinge_loss(labels=one_hot_labels, logits=logits))
...
def create_ner_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    # change output_layer = model.get_pooled_output() -->
    output_layer = model.get_sequence_output()

    """
    todo 是否不需要进行reshape 也是没问题的？？？
    """
    # # add reshape(output_layer) reshape()  by wcy
    # output_layer = tf.reshape(output_layer, [-1, output_layer.shape[-1]])
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            train_op = optimization.create_optimizer(
                total_loss, learning_rate, num_train_steps, num_warmup_steps, use_tpu, FLAGS)
...
def main(_):
    ...
    if FLAGS.do_predict:
        ...
        for global_step, filename in sorted(
            ...
            result = estimator.predict(input_fn=predict_input_fn, checkpoint_path=filename)

            output_predict_file = os.path.join(FLAGS.output_dir, str(global_step) + "_test_results.tsv")
