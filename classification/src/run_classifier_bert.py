...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    with tf.variable_scope("loss"):
        ...
        if FLAGS.use_label_smoothing:
            ...
            one_hot_labels_ls = one_hot_labels - FLAGS.label_smoothing_weight * (one_hot_labels - 1/FLAGS.label_num)
            per_example_loss = -tf.reduce_sum(one_hot_labels_ls * log_probs, axis=-1)
        else:
        ...
        if FLAGS.use_multi_loss:
            ...
            # focal loss
            if FLAGS.use_label_smoothing:
                fl_loss = -tf.reduce_sum(one_hot_labels_ls * (1 - probabilities) ** FLAGS.fl_loss_gama * log_probs)
            else:
                fl_loss = -tf.reduce_sum(one_hot_labels * (1 - probabilities) ** FLAGS.fl_loss_gama * log_probs)
