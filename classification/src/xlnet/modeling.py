    ...
    with tf.variable_scope(scope):
        ...
        if attn_mask is not None:
            ...
            non_tgt_mask = tf.cast(
                (attn_mask + non_tgt_mask[:, :, None, None]) > 0, dtype=tf_float)
        else:
            non_tgt_mask = None
        ...
        if seg_id is not None:
            if untie_r:
                r_s_bias = tf.get_variable(
                    'r_s_bias', [
                        n_layer, n_head, d_head], dtype=tf_float, initializer=initializer)
            else:
                # default case (tie)
                r_s_bias = tf.get_variable(
                    'r_s_bias', [
                        n_head, d_head], dtype=tf_float, initializer=initializer)

            seg_embed = tf.get_variable(
                'seg_embed', [
                    n_layer, 2, n_head, d_head], dtype=tf_float, initializer=initializer)
...
def lm_loss(hidden, target, n_token, d_model, initializer, lookup_table=None,
    ...
    with tf.variable_scope('lm_loss'):
        ...
        if use_tpu:
            ...
            loss = -tf.reduce_sum(tf.nn.log_softmax(logits)
                                  * one_hot_target, -1)
        else:
...
def classification_loss(
        ...
        scope,
        reuse=None,
        return_logits=False, FLAGS=None):
    """
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if FLAGS.use_label_smoothing:
            tf.logging.info("****  label smoothing ****")
            one_hot_target_ls = one_hot_target - FLAGS.label_smoothing_weight * (one_hot_target - 1/FLAGS.label_num)
            loss = -tf.reduce_sum(tf.nn.log_softmax(logits) * one_hot_target_ls, -1)
        else:
            loss = -tf.reduce_sum(tf.nn.log_softmax(logits) * one_hot_target, -1)

        if FLAGS.use_multi_loss:
            tf.logging.info("**** multi loss ****")
            tf.logging.info("**** ce loss ****: " + str(loss))

            # focal loss
            if FLAGS.use_label_smoothing:
                tf.logging.info("****  label smoothing ****")
                fl_loss = -tf.reduce_sum(one_hot_target_ls * (1 - probabilities) ** FLAGS.fl_loss_gama * tf.nn.log_softmax(logits))
            else:
                fl_loss = -tf.reduce_sum(one_hot_target * (1 - probabilities) ** FLAGS.fl_loss_gama * tf.nn.log_softmax(logits))

            tf.logging.info("**** fl loss ****: " + str(fl_loss))

            # hige loss
            hg_loss = tf.reduce_sum(tf.losses.hinge_loss(labels=one_hot_target, logits=logits))
            tf.logging.info("**** hg loss ****: " + str(hg_loss))

            loss = FLAGS.ce_loss_weight * loss + FLAGS.fl_loss_weight * fl_loss + FLAGS.hg_loss_weight * hg_loss
